...
def test_wan_world_model_preserves_expected_shapes() -> None:
    ...
    context_images = torch.rand(2, DYNAMICS_FRAME_LAYOUT.context_pixel_frames, 3, 128, 128)
    ...
    assert latents.shape == (2, 32, 16, 16)
    assert context_latents.shape == (2, 32, DYNAMICS_FRAME_LAYOUT.context_frames, 16, 16)
    assert reconstructed.shape == (2, 3, 128, 128)
    assert next_latents.shape == (2, 32, DYNAMICS_FRAME_LAYOUT.target_frames, 16, 16)
...
def test_wan_world_model_accepts_explicit_action_chunks() -> None:
    ...
    model = WorldModel(resolution=128)
    context_images = torch.rand(1, DYNAMICS_FRAME_LAYOUT.context_pixel_frames, 3, 128, 128)
    context_latents = model.encode_context_frames(context_images, deterministic=True)
    ...
    assert next_latents.shape == (1, 32, DYNAMICS_FRAME_LAYOUT.target_frames, 16, 16)
...
def test_wan_world_model_supports_rectangular_inputs() -> None:
    ...
    latents = model.encode(images, deterministic=True)
    reconstructed = model.decode(latents)
    assert latents.shape == (2, 32, 30, 40)
...
def test_wan_world_model_autoencode_reports_kl_statistics() -> None:
    ...
    assert output.mu.shape == (2, 32, 16, 16)
    assert output.log_var.shape == (2, 32, 16, 16)
...
def test_wan_world_model_rollout_includes_seed_frame() -> None:
    ...
    model = WorldModel(resolution=128)
    seed = torch.rand(1, DYNAMICS_FRAME_LAYOUT.context_pixel_frames, 3, 128, 128)
