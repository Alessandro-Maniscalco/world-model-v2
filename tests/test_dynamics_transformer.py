...
def test_rf_dit_forward_preserves_video_shape() -> None:
    ...
    conditioning_latent_video = torch.randn(2, 4, dynamics.cfg.max_frames, 8, 8)
    noisy_latent_video = torch.randn_like(conditioning_latent_video)
    timesteps = torch.full((2, dynamics.cfg.max_frames), 10.0)
...
def test_rf_dit_forward_preserves_one_context_three_target_shape() -> None:
    """The tiny DiT should preserve shape and finiteness for the aligned 1->3 layout."""
    ...
    conditioning_latent_video = torch.randn(2, 4, 4, 8, 8)
    noisy_latent_video = torch.randn_like(conditioning_latent_video)
    timesteps = torch.full((2, 4), 10.0)
    condition_mask = dynamics.make_condition_mask(
        conditioning_latent_video,
...
def test_rf_dit_rejects_unsupported_learned_temporal_embedding() -> None:
    """The DreamDojo-mechanics DiT should reject the removed learned temporal embedding path."""

    with pytest.raises(ValueError, match="use_learned_temporal_embedding"):
        build_dynamics(use_learned_temporal_embedding=True)


def test_rf_dit_rejects_global_chunk_action_conditioning() -> None:
    """The DreamDojo-mechanics DiT should reject the removed global action mode."""

    with pytest.raises(ValueError, match="action_conditioning_mode='chunk_per_frame'"):
        build_dynamics(action_conditioning_mode="global_chunk")


def test_rf_dit_requires_adaln_lora() -> None:
    """The DreamDojo-mechanics DiT should require AdaLN-LoRA to stay aligned with upstream."""

    with pytest.raises(ValueError, match="use_adaln_lora=True"):
        build_dynamics(use_adaln_lora=False)
...
def test_prepare_training_inputs_uses_full_clip_rf_interpolation() -> None:
    ...
    dynamics = build_dynamics()
    clean_latent_video = torch.randn(2, 4, dynamics.cfg.max_frames, 8, 8)
    ...
    assert prepared.timesteps.shape == (2, dynamics.cfg.max_frames)
    assert prepared.actions.shape == (2, dynamics.cfg.num_action_per_chunk, 4)
...
def test_make_condition_mask_supports_per_item_conditioning_counts() -> None:
    ...
    dynamics = build_dynamics(
        ...
        conditioning_frame_probabilities=(0.5, 0.5),
        validation_conditioning_frame_choices=(1, 2),
        open_rollout_context_frames=1,
    )
    latents = torch.randn(2, 4, dynamics.cfg.max_frames, 8, 8)
    condition_mask = dynamics.make_condition_mask(latents, num_conditional_frames=torch.tensor([1, 2]))
    assert torch.all(condition_mask[0, :, :1] == 1.0)
    assert torch.all(condition_mask[0, :, 1:] == 0.0)
    assert torch.all(condition_mask[1, :, :2] == 1.0)
    assert torch.all(condition_mask[1, :, 2:] == 0.0)
...
def test_recover_clean_latent_video_inverts_rf_parameterization() -> None:
    ...
    dynamics = build_dynamics()
    clean = torch.randn(2, 4, dynamics.cfg.max_frames, 8, 8)
...
def test_forward_rejects_wrong_action_horizon() -> None:
    ...
    noisy_latent_video = torch.randn_like(conditioning_latent_video)
    timesteps = torch.full((1, dynamics.cfg.max_frames), 10.0)
    condition_mask = dynamics.make_condition_mask(
        conditioning_latent_video,
        num_conditional_frames=dynamics.cfg.context_frames,
    )
...
def test_zero_init_action_embedder_starts_as_noop() -> None:
    ...
    dynamics = build_dynamics(zero_init_action_embedder=True)
    clean_latent_video = torch.randn(1, 4, dynamics.cfg.max_frames, 8, 8)
    timesteps = torch.full((1, dynamics.cfg.max_frames), 9.0)
    condition_mask = dynamics.make_condition_mask(
        clean_latent_video,
...
def test_forward_applies_conditional_frame_timestep_override(
    ...
    def fake_forward(
        *,
        ...
        return torch.zeros(1, 4, dynamics.cfg.max_frames, 8, 8)
    ...
    assert torch.allclose(captured["timesteps"][:, :2], torch.full((1, 2), 0.5))
    assert torch.allclose(captured["timesteps"][:, 2:], torch.full((1, 2), 12.0))
...
def test_forward_applies_conditional_frame_sigma_to_repinning(
    ...
    dynamics = build_dynamics(
        context_frames=2,
        target_frames=2,
        conditional_frame_sigma=0.25,
        conditioning_frame_choices=(2,),
        conditioning_frame_probabilities=(1.0,),
        validation_conditioning_frame_choices=(2,),
        open_rollout_context_frames=2,
    )
    conditioning_latent_video = torch.zeros(1, 4, dynamics.cfg.max_frames, 8, 8)
    ...
    assert torch.allclose(captured["x"][:, :, :2], torch.full((1, 4, 2, 8, 8), 0.25))
    assert torch.allclose(captured["x"][:, :, 2:], torch.full((1, 4, 2, 8, 8), 9.0))
...
def test_sample_conditioned_latent_video_uses_cfg_guidance(
    ...
    def fake_forward(
        ...
        output = torch.zeros(1, 4, dynamics.cfg.max_frames, 8, 8)
...
def test_sample_next_latent_supports_three_frame_contexts(
    ...
    """Sampling should also work for an explicitly configured three-frame context."""

    dynamics = build_dynamics(
        context_frames=3,
        target_frames=1,
        conditioning_frame_choices=(3,),
        conditioning_frame_probabilities=(1.0,),
        validation_conditioning_frame_choices=(3,),
        open_rollout_context_frames=3,
    )
    ...
    assert next_latent.shape == (1, 4, 1, 8, 8)
